...
public class BMI {
	...
	public static double[] getBMIValues(List<Person> persons) {
		 ...
		 * 
		 * for(int i=0; i<persons.size(); i++) bmiValues[i] =
		 * calculateBMI(persons.get(i));
		 * 
		 * return bmiValues;
		 */
	}
	...
	public static Person findPersonWithLowestBMI(List<Person> persons) {
		 ...
		 */
		return persons.stream().sorted(Comparator.comparing(BMI::calculateBMI)).reduce((p1, p2) -> p1).get();
	}
